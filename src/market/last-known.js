...
function addCandidate(candidates, candidate, source) {
  ...
  const observedAt = observationTime(candidate?.observedAt || candidate?.asOf || candidate?.date || candidate?.time || candidate?.timestamp);
...
export function lastKnownMarketQuote(assetId, currentMarket, cachedMarket, now = Date.now()) {
  ...
  const currentResponseExists = Boolean(currentMarket?.diagnostics) || Object.keys(currentMarket?.assets || {}).length > 0;
  const cutoff = currentResponseExists && observationTime(currentMarket?.updatedAt)
