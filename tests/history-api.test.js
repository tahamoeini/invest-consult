...
test("history requests accept only known assets and ranges", () => {
  ...
  assert.equal(parseHistoryRequest("https://app.test/api/history?assets=unknown&range=all").error, "no-supported-assets");
...
test("crypto history stays unavailable without the optional Demo key", async () => {
  ...
  assert.equal(requestedUrls.some((url) => url.includes("api.coingecko.com")), false);
});
...
test("a failed local history provider returns partial results and labels the unavailable asset", async () => {
  ...
  const response = await withFetch(async (input) => {
    ...
    if (url.hostname === "www.tgju.org" && url.pathname.endsWith("/price_dollar_rl")) {
      return new Response(tgjuPage([[now - 86400000, 2300000], [now, 2320000]]), { status: 200 });
    }
    if (url.hostname === "www.tgju.org" && url.pathname.endsWith("/geram18")) return new Response("provider unavailable", { status: 503 });
    if (url.hostname === "www.chartgoldprice.com") {
      return new Response(JSON.stringify({ history: { gold: [[now - 86400000, 80], [now, 82]] } }), { status: 200 });
    }
    throw new Error("unexpected-provider " + url.href);
  }, async () => onRequestGet({
